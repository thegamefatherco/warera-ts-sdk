...
* [getById](#getbyid) - Get round by ID
...
<!-- UsageSnippet language="typescript" operationID="round.getById" method="get" path="/round.getById" -->
```typescript
...
import { roundsGetById } from "@thegamefatherco/warera-sdk/funcs/roundsGetById.js";
...
async function run() {
  const res = await roundsGetById(warera, {
    input: {
      roundId: "<id>",
    },
  });
  if (res.ok) {
    ...
    console.log("roundsGetById failed:", res.error);
  }
}
