...
<!-- UsageSnippet language="typescript" operationID="round.getLastHits" method="get" path="/round.getLastHits" -->
```typescript
...
async function run() {
  await warera.round.getLastHits({
    input: {
      roundId: "<id>",
    },
  });
...
import { roundGetLastHits } from "@thegamefatherco/warera-sdk/funcs/roundGetLastHits.js";
...
async function run() {
  const res = await roundGetLastHits(warera, {
    input: {
      roundId: "<id>",
    },
  });
  if (res.ok) {
    ...
    console.log("roundGetLastHits failed:", res.error);
  }
}
