...
<!-- UsageSnippet language="typescript" operationID="region.getById" method="post" path="/region.getById" -->
```typescript
...
import { regionGetById } from "@thegamefatherco/warera-sdk/funcs/regionGetById.js";
...
async function run() {
  const res = await regionGetById(warera);
  if (res.ok) {
    const { value: result } = res;
    
  } else {
    console.log("regionGetById failed:", res.error);
  }
}
