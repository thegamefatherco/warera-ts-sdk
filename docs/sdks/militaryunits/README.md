...
<!-- UsageSnippet language="typescript" operationID="mu.getManyPaginated" method="get" path="/mu.getManyPaginated" -->
```typescript
...
async function run() {
  await warera.militaryUnits.getPaginated();
...
import { militaryUnitsGetPaginated } from "@thegamefatherco/warera-sdk/funcs/militaryUnitsGetPaginated.js";
...
async function run() {
  const res = await militaryUnitsGetPaginated(warera);
  if (res.ok) {
    const { value: result } = res;
    
  } else {
    console.log("militaryUnitsGetPaginated failed:", res.error);
  }
}
