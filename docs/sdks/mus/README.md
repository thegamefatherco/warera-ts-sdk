...
* [getById](#getbyid) - Get military unit by ID
...
<!-- UsageSnippet language="typescript" operationID="mu.getById" method="get" path="/mu.getById" -->
```typescript
...
import { musGetById } from "@thegamefatherco/warera-sdk/funcs/musGetById.js";
...
async function run() {
  const res = await musGetById(warera, {
    input: {
      muId: "<id>",
    },
  });
  if (res.ok) {
    ...
    console.log("musGetById failed:", res.error);
  }
}
