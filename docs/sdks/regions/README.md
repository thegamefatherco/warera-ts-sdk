...
<!-- UsageSnippet language="typescript" operationID="region.getRegionsObject" method="post" path="/region.getRegionsObject" -->
```typescript
...
import { regionsGetAll } from "@thegamefatherco/warera-sdk/funcs/regionsGetAll.js";
...
async function run() {
  const res = await regionsGetAll(warera);
  if (res.ok) {
    const { value: result } = res;
    
  } else {
    console.log("regionsGetAll failed:", res.error);
  }
}
